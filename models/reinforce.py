...
class REINFORCE: # as in Reinforcement Learning by Sutton & Barto (page 326)
    ...
    def train(self, env, n_episodes, alpha, gamma):
        for episode_num in range(n_episodes):
            ...
            while not done:
                ...
                if reward != 0:
                    ...
                    round_returns = [gamma ** (round_length - k - 1) * reward for k in range(round_length)]

                    for r, gradients in zip(round_returns, round_gradients):
                        for i in range(len(self.layers)):
                            self.parameters[f'w{i + 1}'] += alpha * r * gradients[f'dw{i + 1}']
                            self.parameters[f'b{i + 1}'] += alpha * r * gradients[f'db{i + 1}']
